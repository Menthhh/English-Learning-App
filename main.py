...
project = Tk()
project.wm_iconbitmap('./images/icon.png')
    ...
    def sign_up_button(self):
        ...
        #set password length to 8 characters 
            ...
            #save email, username, password to a file
                ...
                file.write(str({'ielts':0,'tofel':0}))
                file.close()
...
class Admin:
    ...
    def delete_user(self):
        #Delete the user that selected in the table and delete the file that contains the user's data
        selected = self.tree.selection()
        for i in selected:
            username = self.tree.item(i)['values'][1]
            self.tree.delete(i)
        #delete from the datasheet.txt file 
        file = open("data/datasheet.txt",'r')
        d = file.read()
        r = ast.literal_eval(d)
        file.close()
